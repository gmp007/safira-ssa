...
- `safira.data`: packaged snapshot loading, World Bank download, panel reading, sample data writing.
...
The default `safira.in` file keeps workflow decisions outside Python code. That is the replacement for editing notebook cells such as “MAIN - call whichever plots you want.” Its `data_mode` option selects the packaged offline snapshot, a fresh World Bank API download, or a custom local panel.
